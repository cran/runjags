...
To illustrate how to use run.jags we will look at the Salmonella example from [chapter 6.5.2](http://www.mrc-bsu.cam.ac.uk/software/bugs/the-bugs-project-the-bugs-book/bugs-book-examples/the-bugs-book-examples-chapter-6-6-5-2/) of this book, with thanks to Lunn, Jackson, Best, Thomas, and Spiegelhalter 2012, for kind permission to reproduce this code.  The model below was taken from the WinBUGS example, modified to include a pair of curly brackets to demarcate the Data and Inits blocks (mandatory), and adding a second Inits block with different starting values for 2 chains:
...
This re-runs the original model, but this time the convergence and sample size is automatically monitored and controlled, and the two chains are run in parallel to speed up lengthy simulations.  Each of the functions run.jags, extend.jags, autorun.jags and autoextend.jags allows a method argument to be specified, with the options available detailed in the [user guide](userguide.pdf).
...
datafit
```
For more details of simulation studies, see the [user guide](userguide.pdf).
...
There is a [sourceforge repository](http://runjags.sourceforge.net/) associated with the runjags package, which contains an alternative compile-free version of the package amongst other things.  The [sourceforge version of this vignette](http://runjags.sourceforge.net/quickjags.html) may also contain additional material not found in the [version hosted on CRAN](http://cran.r-project.org/package=runjags).  This package is in constant development, so if you find any issues or the code does not behave as expected please email the pacckage developer to report the problem.  General comments and suggestions on how you find using this package are also very welcome.
