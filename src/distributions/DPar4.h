...
/**
 * <pre>
 * x ~ dpar3(alpha, sigma, mu, gamma);
 * f(x|sigma,mu,gamma) = (alpha * ((x-mu)/sigma)^((1/gamma) -1) * (((x-mu)/sigma)^(1/gamma) + 1)^(-(alpha+1))) / (gamma*sigma); x >= mu
 * </pre>
 * @short Pareto type IV distribution
 */
class DPar4 : public RScalarDist {
 public:
  DPar4();
